...
import { getBaseUrl, formatSitemapDate, escapeXml, truncateText } from '@/config/sitemap'
...
export async function GET() {
  ...
  try {
    ...
    allUrls.push(...staticPages.map(page => ({
      ...page,
    ...
    if (Array.isArray(activities)) {
      activities.forEach((activity: any) => {
        allUrls.push({
          ...
          lastmod: formatSitemapDate(activity.updatedAt || activity.createdAt || new Date()),
          changefreq: 'weekly',
          priority: 0.8,
          type: 'activity',
          data: activity
    ...
    if (Array.isArray(destinations)) {
      destinations.forEach((destination: any) => {
        ...
        allUrls.push({
          ...
          lastmod: formatSitemapDate(destination.updatedAt || destination.createdAt || new Date()),
          changefreq: 'weekly',
          priority: 0.7,
          type: 'destination',
          data: destination
    ...
    if (Array.isArray(packages)) {
      packages.forEach((pkg: any) => {
        ...
        allUrls.push({
          ...
          lastmod: formatSitemapDate(pkg.updatedAt || pkg.createdAt || new Date()),
          changefreq: 'weekly',
          priority: 0.9,
          type: 'package',
          data: pkg
    ...
    if (Array.isArray(blogs)) {
      ...
      publishedBlogs.forEach((blog: any) => {
        allUrls.push({
          ...
          lastmod: formatSitemapDate(blog.updatedAt || blog.createdAt || new Date()),
          changefreq: 'monthly',
          priority: 0.6,
          type: 'blog',
          data: blog
    ...
    if (Array.isArray(categories)) {
      ...
      activeCategories.forEach((category: any) => {
        ...
        allUrls.push({
          ...
          lastmod: formatSitemapDate(category.updatedAt || category.createdAt || new Date()),
          changefreq: 'monthly',
          priority: 0.5,
          type: 'category',
          data: category
...
${allUrls.map(item => {
  ...
  if (item.type === 'package' && item.data) {
  ...
  } else if ((item.type === 'activity' || item.type === 'destination') && item.data) {
    ...
    const featuredImage = item.data.media?.original || item.data.media?.thumbnail
    const seoImage = item.data.seo?.media?.original || item.data.seo?.media?.thumbnail
